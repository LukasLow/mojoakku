<!--
Design record for mojoakku/base64 — NOT end-user documentation.
...
streaming with a mandatory flush. It is the **design record**, not end-user
...
All 15 entries in this document are `implemented`, and every entry's
`Implementation status:` is `implemented`, after Phase 12/13.
...
for the library design; the implementation and its tests live in the code tree.
...
argument plus `-> Int`) was resolved during implementation and is recorded under
...
- **Tests names the test file that covers the entry.** It is present for every
  entry after the test phase; the field is never removed.
...
- **Status and implementation status are honest.** Every entry is `implemented`
  after Phase 12/13.
...
**One** item remains genuinely open (see **Open** below).
...
- **Streaming padding at a chunk boundary — RESOLVED (fixed).** An earlier
  implementation rejected a padding run that was split across `feed` calls. The
  streaming decoder now retains an incomplete padding run and lets it cross chunk
  boundaries; the padding is judged only at the real end of stream by `finish`.
  `feed…feed; finish()` therefore matches one-shot `decode` of the concatenation
  exactly. Incomplete padding at the true stream end still raises
  `INVALID_PADDING` (under `PaddingMode.STRICT`); a symbol after padding is
  `INVALID_PADDING` and a structurally impossible quantum is `INVALID_LENGTH`.

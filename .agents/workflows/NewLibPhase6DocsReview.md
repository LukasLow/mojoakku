...
## Purpose
Verify that every designed API for `<lib>` has a complete, consistent `<LIB>_DOCS.md` entry and that the docs match the approved design exactly.
...
## Steps
1. Manager starts ONE `reviewer` with: "Review `mojoakku/<lib>/<LIB>_DOCS.md`. Check that every designed API has a documented entry, that each entry has Status, Signature, Semantics, Errors, Tests and Implementation status, and that the docs match the approved design. Report `APPROVED` or `NEEDS_WORK` with findings."
2. reviewer builds the set of designed API members and the set of documented entries and diffs them in both directions: missing entries and undocumented extra entries are both findings.
3. reviewer checks every entry for the required fields: `Status`, `Signature`, `Semantics`, `Errors`, `Tests`, `Implementation status` (and `Rationale`). Any missing field is a finding with `file:line`.
4. reviewer checks field consistency: identical field names and order across all entries; predictable navigation for a low-vision reader.
...
6. reviewer checks that the `Status` values are valid (`planned` / `scaffolded` / `tested` / `implemented` / `benchmarked`) and that no entry claims `implemented` in this phase.
...
- Every entry has all required fields in the fixed order.

...
- `docs`: records the justified edge in the inline `# API-DOCS` shared block.
...
3. `idea-reviewer` checks whether the standard library, `_internal/` code, or an existing sibling library already covers the need.
...
7. `docs` records the accepted edge in the depending library's inline `# API-DOCS` shared block: what is used, why, and which alternative was rejected. The edge is conceptual, never a physical nesting.
...
- Updated inline `# API-DOCS` shared block with the documented dependency edge and its justification.
...
## Review Gate
`reviewer` verifies: (a) the need is real and not satisfiable internally, (b) license and legal status are cleared by `compliance`, (c) maintenance and supply-chain risk are acceptably low, (d) the graph edge is documented in the inline `# API-DOCS` shared block and creates no directory nesting. Any undocumented edge or unknown license => reject.
